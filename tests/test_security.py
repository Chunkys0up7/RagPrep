...
class TestFileValidator:
    ...
    def test_validate_file_mime_type(self):
        ...
        try:
            ...
            with patch('security.mimetypes.guess_type', return_value=("application/exe", None)):
                is_valid, errors = self.validator.validate_file(temp_path)
                assert is_valid is False
                assert len(errors) == 1
                assert "is not allowed" in errors[0]
...
class TestSecurityManager:
    ...
    def test_assess_file_security_with_validation_failure(self):
        ...
        try:
            ...
            assert any("validation failed" in check.details for check in profile.security_checks)
        finally:
            os.unlink(temp_path)
