...
Machine learning continues to evolve and transform various industries."""
        ...
        try:
            ...
            parse_quality = self.quality_system.assess_document_quality(
            ...
            assert all(
                isinstance(chunk, DocumentChunk) for chunk in chunk_result.chunks
            )
            ...
            if metadata_results:
                metadata_quality = self.quality_system.assess_document_quality(
                    metadata_results[0]
                )
            ...
            performance_summary = (
                self.quality_system.performance_monitor.get_performance_summary()
            )
            assert "total_operations" in performance_summary

            print(f"✅ Pipeline test completed successfully!")
            print(
                f"   📄 Parsed document: {len(parse_result.content.text_content)} characters"
            )
            ...
            print(
                f"   ⚡ Operations tracked: {performance_summary.get('total_operations', 0)}"
            )
...
This test validates the complete document processing pipeline."""
    ...
    try:
        ...
        performance_summary = (
            quality_system.performance_monitor.get_performance_summary()
        )
        ...
        print("=" * 30)
        print(
            f"📄 Document parsed: {len(parse_result.content.text_content)} characters"
        )
