...
from metadata_extractors import ExtractionResult, Entity, Topic, Relationship, Summary
...
class TestContentCompletenessAssessor:
    ...
    def test_assess_parsed_content_text_completeness(self):
        ...
        content.text_content = "This is a sample text content with some length to test completeness."
        ...
        text_metric = next((m for m in report.metrics if m.name == "text_completeness"), None)
    ...
    def test_assess_chunk_content_length_optimality(self):
        ...
        chunk.content = "This is a chunk with optimal length between 100 and 1000 characters."
        chunk.metadata = {"chunk_type": "text", "chunk_index": 0}
        chunk.quality_score = 0.8
        
        report = self.assessor.assess_quality(chunk)
        
        length_metric = next((m for m in report.metrics if m.name == "chunk_length_optimality"), None)
    ...
    def test_assess_chunking_result_success(self):
        ...
        success_metric = next((m for m in report.metrics if m.name == "chunking_success"), None)
...
class TestStructureIntegrityAssessor:
    ...
    def test_assess_parsed_structure_variety(self):
        ...
        structure_metric = next((m for m in report.metrics if m.name == "structure_variety"), None)
    ...
    def test_assess_chunk_structure_consistency(self):
        ...
        id_metric = next((m for m in report.metrics if m.name == "chunk_id_validity"), None)
        type_metric = next((m for m in report.metrics if m.name == "chunk_type_consistency"), None)
        index_metric = next((m for m in report.metrics if m.name == "chunk_index_validity"), None)
    ...
    def test_assess_chunking_structure_ordering(self):
        ...
        ordering_metric = next((m for m in report.metrics if m.name == "chunk_ordering"), None)
        strategy_metric = next((m for m in report.metrics if m.name == "strategy_consistency"), None)
...
class TestMetadataAccuracyAssessor:
    ...
    def test_assess_extraction_result_success(self):
        ...
        success_metric = next((m for m in report.metrics if m.name == "extraction_success"), None)
        entity_metric = next((m for m in report.metrics if m.name == "entity_extraction_quality"), None)
        topic_metric = next((m for m in report.metrics if m.name == "topic_extraction_quality"), None)
    ...
    def test_assess_parsed_metadata_completeness(self):
        ...
        completeness_metric = next((m for m in report.metrics if m.name == "metadata_completeness"), None)
        parser_metric = next((m for m in report.metrics if m.name == "parser_validity"), None)
    ...
    def test_assess_chunk_metadata_consistency(self):
        ...
        consistency_metric = next((m for m in report.metrics if m.name == "chunk_metadata_consistency"), None)
        score_metric = next((m for m in report.metrics if m.name == "quality_score_validity"), None)
