...
class DocumentChunker(ABC):
    ...
    def _extract_chunk_metadata(self, chunk_content: str, chunk_index: int, 
    ...
    def _assess_chunk_quality(self, chunk_content: str) -> float:
        ...
        # Combined score
        quality_score = (length_score * 0.4 + completeness_score * 0.4 + richness_score * 0.2)
    ...
    def chunk_document(self, parsed_content: ParsedContent) -> ChunkingResult:
        ...
        try:
            ...
            while start_pos < len(text_content):
                ...
                if len(chunk_content) < min_size and start_pos + min_size < len(text_content):
                ...
                if chunk_content:
                    ...
                    metadata = self._extract_chunk_metadata(chunk_content, chunk_index, "fixed_size")
                    metadata.update({
            ...
            for chunk in chunks:
                if len(chunk.content) < min_size:
                    warnings.append(f"Chunk {chunk.chunk_id} is below minimum size ({len(chunk.content)} < {min_size})")
                elif len(chunk.content) > max_size * 1.2:  # Allow 20% tolerance
                    warnings.append(f"Chunk {chunk.chunk_id} exceeds maximum size ({len(chunk.content)} > {max_size})")
...
class StructuralChunker(DocumentChunker):
    ...
    def chunk_document(self, parsed_content: ParsedContent) -> ChunkingResult:
        ...
        try:
            ...
            if "paragraphs" in structured_content:
                # DOCX-like structure
                chunks.extend(self._chunk_by_paragraphs(structured_content, chunk_index))
                chunk_index = len(chunks)
            ...
            if not chunks:
                logger.warning("No structural elements found, falling back to fixed-size chunking")
            ...
            for chunk in chunks:
                if len(chunk.content) < min_size:
                    warnings.append(f"Chunk {chunk.chunk_id} is below minimum size ({len(chunk.content)} < {min_size})")
                elif len(chunk.content) > max_size * 1.5:  # Allow 50% tolerance for structural chunks
                    warnings.append(f"Chunk {chunk.chunk_id} exceeds maximum size ({len(chunk.content)} > {max_size})")
    ...
    def _chunk_by_pages(self, structured_content: Dict[str, Any], start_index: int) -> List[DocumentChunk]:
        ...
        for i, page in enumerate(pages):
            if isinstance(page, dict) and "text_blocks" in page:
                ...
                if page_text:
                    ...
                    metadata = self._extract_chunk_metadata(page_text, start_index + i, "page")
                    metadata.update({
                        "page_number": i + 1,
                        "page_index": i
                    })
    ...
    def _chunk_by_paragraphs(self, structured_content: Dict[str, Any], start_index: int) -> List[DocumentChunk]:
        ...
        for i, para in enumerate(paragraphs):
            if isinstance(para, dict) and "text" in para:
                ...
                if para_text:
                    ...
                    metadata = self._extract_chunk_metadata(para_text, start_index + i, "paragraph")
                    metadata.update({
                        "paragraph_index": i,
        ...
        for i, heading in enumerate(headings):
            if isinstance(heading, dict) and "text" in heading:
                ...
                if current_section and current_heading:
                    ...
                    if section_text:
                        chunk_id = self._generate_chunk_id(section_text, start_index + len(chunks))
                        metadata = self._extract_chunk_metadata(section_text, start_index + len(chunks), "section")
                        metadata.update({
            ...
            if len(current_text) > self.chunking_config.max_chunk_size:
                ...
                if len(current_chunk) > 1:
                    ...
                    if chunk_text:
                        ...
                        metadata = self._extract_chunk_metadata(chunk_text, chunk_index, "semantic")
    ...
    def chunk_document(self, parsed_content: ParsedContent) -> ChunkingResult:
        ...
        try:
            ...
            if structural_result.success and len(structural_result.chunks) > 0:
                ...
                avg_quality = sum(chunk.quality_score for chunk in structural_result.chunks) / len(structural_result.chunks)
                ...
                if avg_quality >= 0.7:  # Good quality threshold
                    ...
                    logger.info("Structural chunks quality too low, trying semantic chunking")
                    warnings.append("Structural chunking quality below threshold, falling back to semantic")
            ...
            if semantic_result.success and len(semantic_result.chunks) > 0:
                ...
                avg_quality = sum(chunk.quality_score for chunk in semantic_result.chunks) / len(semantic_result.chunks)
                ...
                if avg_quality >= 0.6:  # Lower threshold for semantic chunks
                    ...
                    logger.info("Semantic chunks quality too low, using fixed-size chunking")
                    warnings.append("Semantic chunking quality below threshold, falling back to fixed-size")
...
class DocumentChunkerFactory:
    ...
    def create_chunker(strategy: str, config: Config) -> DocumentChunker:
        ...
        if strategy == "fixed":
            ...
            logger.warning(f"Unknown chunking strategy '{strategy}', falling back to hybrid")
            return HybridChunker(config)


# Convenience function to get chunker instance
def get_document_chunker(strategy: str = "hybrid", config_path: Optional[str] = None) -> DocumentChunker:
