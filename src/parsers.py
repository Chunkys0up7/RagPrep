            ...
            for block in text_dict["blocks"]:
                if "lines" in block:
                    ...
                    if len(block["lines"]) > 1:
                        ...
                        if len(table_data) > 1:  # At least 2 rows to be considered a table
                            tables.append({
                                "data": table_data,
                                "bbox": block["bbox"],
...
class DOCXParser(DocumentParser):
    ...
    def can_parse(self, file_path: str) -> bool:
        ...
        return Path(file_path).suffix.lower() in [".docx", ".doc"] and self.docx_available
    ...
    def parse(self, file_path: str) -> ParserResult:
        ...
        try:
            ...
            content = ParsedContent(
                ...
                structure={"paragraphs": len(structured_content["paragraphs"]), "tables": len(structured_content["tables"])},
                parser_used=self.parser_name,
                parsing_errors=[],
                parsing_warnings=[]
            )
...
class HTMLParser(DocumentParser):
    ...
    def can_parse(self, file_path: str) -> bool:
        ...
        return Path(file_path).suffix.lower() in [".html", ".htm"] and self.bs4_available
...
class CascadingDocumentParser:
    ...
    def _initialize_parsers(self):
        ...
        self.parsers = [parser for parser in self.parsers if self._is_parser_available(parser)]
    ...
    def parse_document(self, file_path: str) -> ParserResult:
        ...
        suitable_parsers = [parser for parser in self.parsers if parser.can_parse(file_path)]
        ...
        for parser in suitable_parsers:
            ...
            try:
                ...
                if result.success:
                    logger.info(f"Successfully parsed {file_path} with {parser.parser_name}")
                    return result
                else:
                    logger.warning(f"Parser {parser.parser_name} failed: {result.error_message}")
        ...
        return ParserResult(
            ...
            parser_name="CascadingParser",
            processing_time=0.0,
            metadata={"file_extension": file_ext, "attempted_parsers": [p.parser_name for p in suitable_parsers]}
        )
