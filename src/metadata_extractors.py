...
class MetadataExtractor(ABC):
    ...
    def _validate_content(self, content: Union[str, ParsedContent, DocumentChunk]) -> str:
    ...
    def _assess_extraction_quality(self, entities: List[Entity], topics: List[Topic], 
...
class BasicMetadataExtractor(MetadataExtractor):
    ...
    def extract_metadata(self, content: Union[str, ParsedContent, DocumentChunk]) -> ExtractionResult:
        ...
        try:
            ...
            # Assess quality
            quality_score = self._assess_extraction_quality(entities, topics, relationships)
    ...
    def _extract_basic_topics(self, text: str) -> List[Topic]:
        ...
        topic_keywords = {
            "technology": ["software", "hardware", "computer", "system", "technology", "digital", "data"],
            "business": ["business", "company", "market", "industry", "management", "strategy", "finance"],
            "science": ["research", "study", "analysis", "experiment", "scientific", "method", "theory"],
            "health": ["health", "medical", "treatment", "patient", "disease", "medicine", "clinical"],
            "education": ["education", "learning", "teaching", "student", "school", "course", "training"]
        }
    ...
    def _extract_basic_relationships(self, text: str, entities: List[Entity]) -> List[Relationship]:
...
class LLMMetadataExtractor(MetadataExtractor):
    ...
    def extract_metadata(self, content: Union[str, ParsedContent, DocumentChunk]) -> ExtractionResult:
        ...
        try:
            ...
            if not self.openai_api_key:
                warnings.append("OpenAI API key not available, falling back to basic extraction")
    ...
    def _extract_relationships_with_llm(self, text: str, entities: List[Entity]) -> List[Relationship]:
        ...
        try:
            ...
            entity_list = [f"{e.text} ({e.entity_type})" for e in entities[:10]]  # Limit entities
...
class MetadataExtractorFactory:
    ...
    def create_extractor(extraction_level: str, config: Config) -> MetadataExtractor:
        ...
        if extraction_level == "basic":
            ...
            logger.warning(f"Unknown extraction level '{extraction_level}', falling back to basic")
            return BasicMetadataExtractor(config)


# Convenience function to get extractor instance
def get_metadata_extractor(extraction_level: str = "enhanced", config_path: Optional[str] = None) -> MetadataExtractor:
