...
class QualityReport:
    ...
    def __post_init__(self):
        ...
        if total_weight > 0:
            self.overall_score = sum(metric.score * metric.weight for metric in self.metrics) / total_weight
        else:
...
class ContentCompletenessAssessor(QualityAssessor):
    ...
    def assess_quality(self, content: Union[ParsedContent, DocumentChunk, ChunkingResult]) -> QualityReport:
    ...
    def _assess_parsed_content(self, content: ParsedContent) -> List[QualityMetric]:
        ...
        if content.text_content:
            ...
            if text_length > 0:
                text_score = min(1.0, text_length / 1000)  # Normalize to 1.0 at 1000 chars
                metrics.append(QualityMetric(
        ...
        if content.structure:
            structure_score = min(1.0, len(content.structure) / 10)  # Normalize to 1.0 at 10 elements
            metrics.append(QualityMetric(
        ...
        if content.metadata:
            metadata_score = min(1.0, len(content.metadata) / 5)  # Normalize to 1.0 at 5 metadata items
            metrics.append(QualityMetric(
        ...
        if content.parsing_errors:
            error_score = max(0.0, 1.0 - len(content.parsing_errors) * 0.2)  # Reduce score for each error
            metrics.append(QualityMetric(
                name="error_freedom",
                score=error_score,
                weight=1.5,
                details={"error_count": len(content.parsing_errors), "errors": content.parsing_errors}
            ))
        else:
            metrics.append(QualityMetric(
                name="error_freedom",
    ...
    def _assess_chunk_content(self, content: DocumentChunk) -> List[QualityMetric]:
        ...
        if content.content:
            ...
            if content_length > 0:
                ...
                metrics.append(QualityMetric(
                    ...
                    score=length_score,
                    weight=1.5,
                    details={"content_length": content_length, "optimal_range": "100-1000"}
                ))
            else:
                metrics.append(QualityMetric(
                    name="chunk_length_optimality",
        ...
        # Metadata completeness
        if content.metadata:
            metadata_score = min(1.0, len(content.metadata) / 3)  # Normalize to 1.0 at 3 metadata items
            metrics.append(QualityMetric(
    ...
    def _assess_chunking_result(self, content: ChunkingResult) -> List[QualityMetric]:
        ...
        if content.processing_time:
            ...
            metrics.append(QualityMetric(
                ...
                score=time_score,
                weight=1.0,
                details={"processing_time": content.processing_time, "optimal_threshold": 30}
            ))
        
        # Error assessment
        if content.errors:
            error_score = max(0.0, 1.0 - len(content.errors) * 0.3)  # Reduce score for each error
            metrics.append(QualityMetric(
                name="error_freedom",
                score=error_score,
                weight=1.5,
                details={"error_count": len(content.errors), "errors": content.errors}
            ))
        else:
            metrics.append(QualityMetric(
                name="error_freedom",
...
class StructureIntegrityAssessor(QualityAssessor):
    ...
    def _assess_parsed_structure(self, content: ParsedContent) -> List[QualityMetric]:
        ...
        if content.structure:
            ...
            if structure_items > 0:
                ...
                has_headings = any('heading' in str(item).lower() for item in content.structure)
                has_paragraphs = any('paragraph' in str(item).lower() for item in content.structure)
                has_sections = any('section' in str(item).lower() for item in content.structure)
                
                structure_variety = sum([has_headings, has_paragraphs, has_sections]) / 3
                metrics.append(QualityMetric(
        ...
        if content.text_content:
            ...
            if len(non_empty_lines) > 1:
                ...
                heading_patterns = sum(1 for line in non_empty_lines if line.isupper() or line.startswith('#'))
    ...
    def _assess_chunk_structure(self, content: DocumentChunk) -> List[QualityMetric]:
        ...
        if content.chunk_type:
            type_validity = 1.0 if content.chunk_type in ['text', 'table', 'image', 'mixed'] else 0.5
            metrics.append(QualityMetric(
    ...
    def _assess_chunking_structure(self, content: ChunkingResult) -> List[QualityMetric]:
        """Assess chunking result structure integrity."""
        metrics = []
        
        # Strategy consistency
        if content.chunking_strategy:
            strategy_validity = 1.0 if content.chunking_strategy in ['fixed', 'structural', 'semantic', 'hybrid'] else 0.5
            metrics.append(QualityMetric(
        ...
        if content.chunks and len(content.chunks) > 1:
            ...
            indices = [chunk.chunk_index for chunk in content.chunks if chunk.chunk_index is not None]
            if indices:
            ...
            metrics.append(QualityMetric(
                ...
                weight=1.0,
                details={"chunk_count": len(content.chunks) if content.chunks else 0}
            ))
...
class MetadataAccuracyAssessor(QualityAssessor):
    ...
    def assess_quality(self, content: Union[ExtractionResult, ParsedContent, DocumentChunk]) -> QualityReport:
    ...
    def _assess_extraction_result(self, content: ExtractionResult) -> List[QualityMetric]:
        ...
        if content.processing_time:
            ...
            metrics.append(QualityMetric(
                ...
                score=time_score,
                weight=1.0,
                details={"processing_time": content.processing_time, "optimal_threshold": 60}
            ))
    ...
    def _assess_parsed_metadata(self, content: ParsedContent) -> List[QualityMetric]:
        ...
        if content.metadata:
            ...
            if metadata_count > 0:
                ...
                present_fields = sum(1 for field in essential_fields if field in content.metadata)
        ...
        if content.parser_used:
            parser_score = 1.0 if content.parser_used in ['marker', 'pymupdf', 'unstructured', 'python-docx', 'beautifulsoup'] else 0.5
            metrics.append(QualityMetric(
    ...
    def _assess_chunk_metadata(self, content: DocumentChunk) -> List[QualityMetric]:
        ...
        if content.metadata:
            ...
            if metadata_count > 0:
                ...
                present_fields = sum(1 for field in chunk_fields if field in content.metadata)
        ...
        if hasattr(content, 'quality_score') and content.quality_score is not None:
            ...
            metrics.append(QualityMetric(
                ...
                weight=0.5,
                details={"quality_score": content.quality_score, "valid_range": "0.0-1.0"}
            ))
        else:
            metrics.append(QualityMetric(
                name="quality_score_validity",
...
class QualityAssessmentSystem:
    ...
    def _generate_recommendations(self, metrics: List[QualityMetric]) -> List[str]:
        ...
        for metric in metrics:
            if not metric.passed:
                if metric.name == "text_completeness":
                    recommendations.append("Consider using a different parser or document format for better text extraction")
                elif metric.name == "structure_completeness":
                    recommendations.append("Document structure could be improved with better formatting or parsing")
                elif metric.name == "chunk_length_optimality":
                    recommendations.append("Adjust chunking parameters for optimal chunk sizes")
                elif metric.name == "error_freedom":
                    recommendations.append("Review and fix parsing errors to improve document quality")
                elif metric.name == "extraction_success":
                    recommendations.append("Check extraction configuration and input document quality")
                elif metric.name == "processing_efficiency":
                    recommendations.append("Consider optimizing processing parameters or using more efficient strategies")
        ...
        if len(failed_metrics) > len(metrics) * 0.3:  # More than 30% failed
            recommendations.append("Overall document quality is low. Consider preprocessing or using different source documents")
...
def get_quality_assessment_system(config_path: Optional[str] = None) -> QualityAssessmentSystem:
